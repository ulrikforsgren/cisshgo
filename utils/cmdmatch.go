...
func CompileCommands(supportedCommands map[string]string) (*MatchCommands, error) {
	...
	for cmd, file := range supportedCommands {
		...
		// TODO: Improve the perDeviceData mechanism. Far from perfect...
		cmdData := map[int]string{}
		perDeviceData := false
		if strings.Contains(file, "<PORT>") {
			perDeviceData = true
		} else {
			cmdData[0] = readFile(file)
		}
		fieldsMap = append(fieldsMap, &CommandPattern{Pattern: comp_fields,
			File: CommandFile{Name: file, PerDeviceData: perDeviceData, CmdData: cmdData}})
	}
